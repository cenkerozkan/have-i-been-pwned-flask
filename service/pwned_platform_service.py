...
class PwnedPlatformService:
    ...
    def get_by_email_id(self, email_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            result["success"] = True
            result["message"] = f"Successfully retrieved pwned platforms for email ID {email_id}"
            result["data"] = {"platforms": platforms_json}
            
        except Exception as e:
            result["success"] = False
            result["message"] = f"Failed to retrieve pwned platforms for email ID {email_id}"
            result["error"] = str(e)
            self._logger.error(f"Failed to retrieve pwned platforms for email ID {email_id}: {str(e)}")
